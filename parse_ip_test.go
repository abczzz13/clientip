...
func TestParseIP(t *testing.T) {
	tests := []struct {
		...
		{name: "valid IPv4", input: "203.0.113.1", want: netip.MustParseAddr("203.0.113.1")},
		{name: "valid IPv4 with leading whitespace", input: "  203.0.113.1", want: netip.MustParseAddr("203.0.113.1")},
		{name: "valid IPv4 with trailing whitespace", input: "203.0.113.1  ", want: netip.MustParseAddr("203.0.113.1")},
		{name: "valid IPv4 with surrounding whitespace", input: "  203.0.113.1  ", want: netip.MustParseAddr("203.0.113.1")},
		{name: "valid IPv4 with tabs", input: "\t203.0.113.1\t", want: netip.MustParseAddr("203.0.113.1")},
		{name: "valid IPv4 with port", input: "203.0.113.1:8080", want: netip.MustParseAddr("203.0.113.1")},
		{name: "valid IPv4 with double quotes", input: `"203.0.113.1"`, want: netip.MustParseAddr("203.0.113.1")},
		{name: "valid IPv4 with single quotes", input: "'203.0.113.1'", want: netip.MustParseAddr("203.0.113.1")},
		{name: "valid IPv4 with quotes and port", input: `"203.0.113.1:8080"`, want: netip.MustParseAddr("203.0.113.1")},
		...
		{name: "localhost IPv6", input: "::1", want: netip.MustParseAddr("::1")},
		{name: "localhost IPv6 with brackets and port", input: "[::1]:8080", want: netip.MustParseAddr("::1")},
		{name: "empty string", input: "", wantErr: true},
		{name: "whitespace only", input: "   ", wantErr: true},
		{name: "quotes only", input: `""`, wantErr: true},
		{name: "unmatched leading double quote", input: `"203.0.113.1`, wantErr: true},
		{name: "unmatched trailing double quote", input: `203.0.113.1"`, wantErr: true},
		{name: "unmatched leading single quote", input: "'203.0.113.1", wantErr: true},
		{name: "unmatched trailing single quote", input: "203.0.113.1'", wantErr: true},
		{name: "invalid IP", input: "not-an-ip", wantErr: true},
		{name: "invalid IPv4", input: "999.999.999.999", wantErr: true},
		{name: "port only", input: ":8080", wantErr: true},
		{name: "brackets only", input: "[]", wantErr: true},
		{name: "unmatched leading bracket", input: "[2001:db8::1", wantErr: true},
		{name: "unmatched trailing bracket", input: "2001:db8::1]", wantErr: true},
	}
...
func Test_parseRemoteAddr(t *testing.T) {
	tests := []struct {
		...
		{name: "quoted ipv4 fallback", input: `"203.0.113.1"`, want: netip.MustParseAddr("203.0.113.1")},
		{name: "hostname with port", input: "example.com:443", wantErr: true},
		{name: "non-numeric port is ignored", input: "203.0.113.1:notaport", want: netip.MustParseAddr("203.0.113.1")},
...
func TestTrimMatchedChar(t *testing.T) {
	tests := []struct {
		...
		{name: "matching double quote delimiter", input: `"203.0.113.1"`, ch: '"', want: "203.0.113.1"},
		{name: "matching single quote delimiter", input: "'203.0.113.1'", ch: '\'', want: "203.0.113.1"},
		{name: "non-matching delimiter", input: "203.0.113.1", ch: '"', want: "203.0.113.1"},
		{name: "too short to trim", input: `"`, ch: '"', want: `"`},
	}
...
func TestTrimMatchedPair(t *testing.T) {
	tests := []struct {
		...
		{name: "matching pair", input: "[2001:db8::1]", start: '[', end: ']', want: "2001:db8::1"},
		{name: "unmatched leading bracket", input: "[2001:db8::1", start: '[', end: ']', want: "[2001:db8::1"},
		{name: "unmatched trailing bracket", input: "2001:db8::1]", start: '[', end: ']', want: "2001:db8::1]"},
		{name: "too short to trim", input: "[", start: '[', end: ']', want: "["},
	}
