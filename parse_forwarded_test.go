...
func TestParseForwardedValues(t *testing.T) {
	tests := []struct {
		...
		{name: "empty header value", values: []string{""}, wantErr: true},
		{name: "whitespace only header value", values: []string{"   "}, wantErr: true},
		{name: "leading comma creates empty element", values: []string{", for=1.1.1.1"}, wantErr: true},
		{name: "trailing comma creates empty element", values: []string{"for=1.1.1.1,"}, wantErr: true},
		...
		{name: "empty line among multiple header lines is malformed", values: []string{"for=1.1.1.1", "   "}, wantErr: true},
...
func TestParseForwardedValues_MalformedParameterMatrix(t *testing.T) {
	tests := []struct {
		...
		{name: "leading semicolon creates empty parameter", values: []string{";for=1.1.1.1"}},
		{name: "trailing semicolon creates empty parameter", values: []string{"for=1.1.1.1;"}},
		{name: "double semicolon creates empty parameter", values: []string{"for=1.1.1.1;;proto=https"}},
		{name: "whitespace only empty parameter", values: []string{"for=1.1.1.1;   ;proto=https"}},
